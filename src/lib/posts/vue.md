...
Take the imperative JS code in [Code Sample A](#code-sample-a) versus the declarative Vue code in [Code Sample B](#code-sample-b). While declarative is not necessarily _worse_ than imperative, it usually leads to a messier code with strict guidelines. BackboneJS added those guidelines, but it still leads to bulky, hard-to-read code, as seen in their To-do list example. Conversely, Vue took the more straightforward approach: Make the framework work for you. These solutions provided _were_ great, but now we have moved on from that.

Let us look at modern Vue. The Options API is bulky. It also creates many opportunities to shoot yourself in the foot. In the above example, if you were to replace `increment` with an arrow function, it would no longer work. Vue 3 saw these issues and fixed them through the *Composition API*. This API was effectively stolen from another framework, React. React created _React Hooks_ and Vue transformed them into the Composition API. The composition API is far learner and has less boilerplate, as seen in [Code Sample C](#code-sample-c). This code is much simpler, but it has its flaws. We have to tell Vue that we want the variable to be reactive, and we have to import this functionality from Vue, which is just boilerplate. Vue also changes many things when using `ref`, which can cause many compatibility issues.
...
This issue is so big that others have made their solutions, but these also have their struggles. Look at [Vue Macro's _reactivity transform_](https://vue-macros.sxzz.moe/features/reactivity-transform.html). It is a compile-time plugin that wraps around `ref`, creating `$ref`. `$ref` can be used as a normal variable, and at compile time, it gets translated back to `ref`. This transformation still causes issues, though, as seen in [Code Sample D](#code-sample-d). `$ref` values stop being reactive when returned from a function because they get changed to `ref.value`. `.value` is not the proxy object, so it is not reactive. Instead, We have to do `return $$({ x, y })`. We still have the issues of boilerplate, and we still have the woes of proxy objects. All of these issues go against Vue's motto of being simple and lightweight. Vue 3 still allows you to use the options API if you do not want to deal with these proxy objects, but allowing both options has its issues.

The Options and Setup API are compatible, but in a strange way. First of all, it is only 1-way compatibility. Options API code can call into the `setup()` function, but the `setup()` function cannot access methods and data from the Options API (see [Code Sample E](#code-sample-e)). It is more meant as a migration tool than direct compatibility, but more is needed to help the situation. If a person wants to migrate a codebase to the Composition API, they may call their existing code in your Composition API code, which is currently disallowed. The composition API caused a massive ecosystem split between Vue 2 and 3 code, which affects the whole ecosystem.
...
While Vue is good, it has been outshone by its competitors. It started as an extremely simple and relatively fast framework, but years of technical debt have now accumulated. Having two different ways of doing the same thing with various foot guns in each prevents it from being nearly boilerplates. A split ecosystem prevents companies from adopting its latest version, and it is now being beaten at what was its own game: being a simple, fast framework. While it is still an excellent choice, there are far better choices out there than when it was first made.
...
### Code Sample B
Declarative Vue using the Options API. This is what you would write in Vue 1/2
```html
<div id="app">
...
### Code Sample D
Reactivity Transform API
```js
function useMouse() {
  const x = $ref(0)
  const y = $ref(0)
  // do some stuff to change x and y via events
  // x and y are no longer reactive!
  return { x, y }
}
```

gets transformed/unwrapped into
...
### Code Sample G
Sample of Svelte's reactivity update code
```js
...
### Code Sample I
Vue Transition API vs. Svelte's transition API
