...
function convertToNestedObject(headings: readonly DataItemWithChildren[]): DataItemWithChildren {
    const [potentialRoots, rest] = A.partition(headings, (value) => value.depth === 1);
...
function recurseDepth(
    ...
    const firstAtLevelIndex = headingsToRecurse.findIndex((value) => value.depth === level);
    ...
    const secondAtLevelIndex = rest.findIndex((value) => value.depth === level);
...
export const load: PageLoad = async ({params}) => {
    ...
    const cannotFindPost = !post?.metadata || !post.metadata.published || !post.metadata.title;
    if (cannotFindPost) {
...
export const entries: EntryGenerator = (async () => {
    ...
    return posts.filter((post) => post.published).map(D.selectKeys(['slug']));
